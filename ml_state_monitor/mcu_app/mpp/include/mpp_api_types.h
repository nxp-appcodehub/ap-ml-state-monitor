...
typedef struct {
    ...
    int frequency; /*!< frequency per second */
} mpp_accelerometer_params_t;

/** Static accelerometer data parameters */
typedef struct {
    int height;
    int width;
    int channel; /*!< channel number */
    int format; /*!< data format */
    int frequency; /*!< frequency per second */
} mpp_static_accel_params_t;

/** SD card parameters */
typedef struct {
    int period_sync; /*!<The periodicity for capture sync */
} mpp_sd_params_t;
...
typedef enum {
    ...
    MPP_ELEMENT_COPY,       /*!< Copy input to output */
...
#define INPUT_DEQUEUE_FREQUENCY	320
