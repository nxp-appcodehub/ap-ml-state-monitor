...
MPP_GLOW_MEM_ALIGN(GLOW_MODEL_MEM_ALIGN)
		...
		model_param_t *param)
{
	...
	if (param->inference_params.constant_weight_MemSize > GLOW_MODEL_CONSTANT_MEM_SIZE)
	{
	    HAL_LOGE("--HAL_VisionAlgoDev_Glow_Init: "
	            "ERROR: constant weights memory size (%d) exceeds max limit (%d)\n",
	            (int)(param->inference_params.constant_weight_MemSize), GLOW_MODEL_CONSTANT_MEM_SIZE);
	    ret = kStatus_HAL_ValgoInitError ;
	    return ret;
	}
	if (param->inference_params.mutable_weight_MemSize > GLOW_MODEL_MUTABLE_MEM_SIZE)
	{
	    HAL_LOGE("--HAL_VisionAlgoDev_Glow_Init: "
	            "ERROR: mutable weights memory size (%d) exceeds max limit (%d)\n",
	            (int)(param->inference_params.mutable_weight_MemSize), GLOW_MODEL_MUTABLE_MEM_SIZE);
	    ret = kStatus_HAL_ValgoInitError ;
	    return ret;
	}
	if (param->inference_params.activations_MemSize > GLOW_MODEL_ACTIVATIONS_MEM_SIZE)
	{
	    HAL_LOGE("--HAL_VisionAlgoDev_Glow_Init: "
	            "ERROR: activations memory size (%d) exceeds max limit (%d)\n",
	            (int)(param->inference_params.activations_MemSize), GLOW_MODEL_ACTIVATIONS_MEM_SIZE);
	    ret = kStatus_HAL_ValgoInitError ;
	    return ret;
	}
	/* Init the device */
	...
	glow_model_param->model_in_mean = param->model_input_mean;
	glow_model_param->model_in_std = param->model_input_std;
...
static hal_valgo_status_t HAL_VisionAlgoDev_Glow_Run(const vision_algo_dev_t *dev, void *data)
{
    ...
    GLOW_MODEL_NormalizationInput((float *) glow_model_param->inputAddr,
           glow_model_param->inputParams.height,
		   glow_model_param->inputParams.num_channels,
		   glow_model_param->inputParams.type,
		   glow_model_param->model_in_mean,
		   glow_model_param->model_in_std);
