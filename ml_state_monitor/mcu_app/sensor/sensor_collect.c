...
#include "vdset_3_vd3_clog.h"
#include "vdset_3_vd3_friction.h"
#include "vdset_3_vd3_off.h"
#include "vdset_3_vd3_on.h"
...
static uint32_t vdset_samples = vdset_3_vd3_clog_samples;
#endif
...
typedef struct _sensor_data
{
    uint64_t sampleNum;
    uint64_t ts_us;
    fxos8700_accelmagdata_t rawDataSensor;
    int8_t temperature;
} sensor_data_t;
...
void SENSOR_Collect_LogExt_Task(void *pvParameters)
{
    ...
    bufSizeLog = snprintf(buf, bufSize, "class,time[ms],ax,ay,az,Bx,By,Bz,T\r\n");
    ...
    while (1)
    {
        ...
        if (NULL != g_sensorCollectQueue && xQueueReceive(g_sensorCollectQueue, &sensorData, portMAX_DELAY) == pdPASS )
        {
            ...
            for (int i = 0; i < CLSF_CHANNELS; i++)
            {
                ...
                sens_val = (float)*(&sensorData.rawDataSensor.accel[0] + i)/SENSOR_ACC_RESOLUTION;
#else
...
static void SENSOR_Collect_TimerCB(TimerHandle_t xTimer)
{
    ...
    for (int i = 0; i < CLSF_CHANNELS; i++)
    {
        *(&sensorData.rawDataSensor.accel[0] + i) = *(vdset_ptr + i * vdset_samples + sensorData.sampleNum);
    }
...
#else
    SENSOR_Run(&rawDataGyro, &rawDataAccel, &temperature);
    sensorData.sampleNum++;
    sensorData.rawDataSensor = rawDataAccel;
