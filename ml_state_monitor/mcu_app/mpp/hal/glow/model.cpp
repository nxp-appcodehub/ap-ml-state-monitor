...
void GLOW_MODEL_NormalizationInput(float* data, int height , int num_channel , mpp_tensor_type_t type, float* mean, float* std)
{
    switch (type)
    {
        case MPP_TENSOR_TYPE_UINT8:
        ...
        case MPP_TENSOR_TYPE_FLOAT32:
        	...
        	{
        		for (uint32_t j = 0 ; j < num_channel ; j++)
        		{
        			(data)[num_channel*i+j] =(data[num_channel*i+j]) - mean[j] ;
        			(data)[num_channel*i+j] =(data[num_channel*i+j]) / std[j];
        		}
        	}
